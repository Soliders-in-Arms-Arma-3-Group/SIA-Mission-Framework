...
[date, overcast, rain, lightnings, waves, windStr, fogParams] params ["_date", "_overcast", "_rain", "_lightnings", "_waves", "_wind", "_fogParams"];

private _diaryText = "";
private _startTime = missionStart;

// Start time & date
private _diaryText = _diaryText + format ["<font size='18'><font color='#aaaaaa'>Mission start time:</font> %1:%2</font><br/>", // aaaaaa is gray
	(if (_startTime # 3 >= 10) then { "" } else { "0" }) + str (_startTime # 3),
	(if (_startTime # 4 >= 10) then { "" } else { "0" }) + str (_startTime # 4)];
